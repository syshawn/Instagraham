...
<html lang="en">
	...
	<body>
        ...
        // set-up query string
        $q = 'SELECT idphoto, title, imageurl, comment FROM photo WHERE idphoto=' . $_GET['id'] . ';';
        ...
        if ($res = $mysqli->query($q)) {
            ...
            if ($res->data_seek(0)) {
                while ($row = $res->fetch_assoc()) {
                    ...
                    //edit the titile
                    echo '<form action="edit-message2.php" method="post" enctype="multipart/form-data">';
                        echo '<div class="text-group" style="margin-left: 230px; margin-top: 30px;">';
                            echo '<input class="form-control" placeholder="imgtitle" type="text" name="imgtitle" value="' . $row['title'] . '">';
                            echo '<label>Title</label>';
                        echo '</div>';
                        
                        //edit the comment
                        echo '<div class="text-group" style="margin-left:230px">';
                            echo '<input class="form-control" placeholder="imgcomment" type="text" name="imgcomment" value="' . $row['comment'] . '">';
                            echo '<label>Write your comment here...</label>';
                        echo '</div>';
                        
                        //hidden idphoto & submit button
                        echo '<p><input type="hidden" name="imgid" value="' . $row['idphoto'] . '">';
